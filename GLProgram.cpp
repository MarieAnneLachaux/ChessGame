...
/// Returns 1 if an OpenGL error occurred, 0 otherwise.
        ...
        void Program::setUniform1u(GLint location, GLint value) {
                use ();
    ...
    GLint Program::getAttribLocation (const std::string & attribName) {
        use();
    const GLchar * cname = attribName.c_str ();
    GLint loc = glGetAttribLocation(_id, cname);
    if (loc == -1)
        throw Exception (std::string ("Program Error: No such attrib named ") + attribName);
    printOpenGLError ("Wrong Attrib Variable [" + attribName + "] for Program [" + name () + "]");
    return loc;
    }


    void Program::setAttrib1f(GLint location, float o){
    use();
        glVertexAttrib1f(location, o);    }

    void Program::setAttrib3f(GLint location, float r, float g, float b) {
    use();
    glVertexAttrib3f(location, r, g, b);
    
    }

    void Program::setAttrib2f(GLint location, float vtX, float vtY){
    use();
    glVertexAttrib2f(location, vtX, vtY);
    }
